...
export default async function handler(req, res) {
  ...
  switch (event) {
    case "push": {
      const { repository, ref, commits, pusher, compare } = payload;
      ...
      embeds.push({
        author,
        title: `📦 Pushed to \`${repository.name}:${shortBranch}\``,
        description: `**By:** ${pusher.name || pusher.email}\n${compare ? `[View Changes](${compare})` : ""}\n\n${commitLines}`,
        color: 0x00ccff,
